...
# 使用SVM训练模型，高斯rbf核函数
svm_clf_rbf = SVC(kernel="rbf", C=6, degree=2, gamma="auto")

start = time()
svm_clf_rbf.fit(sample_data_13, label_train)
print("train_spend_time =  %.2f seconds" % (time() - start))


if __name__ == "__main__":
    # 交叉验证分数，使用accuracy
    print("cross_val_score: ", cross_val_score(svm_clf_rbf, sample_data_13, label_train,
                                               cv=5, scoring="accuracy"))

    # 返回交叉验证每个折叠的预测
    label_train_pred = cross_val_predict(svm_clf_rbf, sample_data_13, label_train, cv=5)
